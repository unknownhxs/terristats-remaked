...
class WinsLBView(discord.ui.View):
    ...
    async def previous_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.page > 0:
            self.page -= 1
            await interaction.response.edit_message(embed=self.create_embed(), view=WinsLBView(self.rankings, self.page))
    ...
    def create_embed(self):
        ...
        embed = discord.Embed(
            ...
            description="Top 100 clans by total wins",
            color=0x2b2d31
        )
...
class WinsLB(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @app_commands.command(name='wins_lb', description='Top 100 clans by total wins')
        ...
        try:
            ...
            if not results:
                await interaction.followup.send(embed=discord.Embed(
                    title="🏆 Wins Leaderboard", description="No data found.", color=0xff0000
                ))
                return
            ...
            if not rankings:
                await interaction.followup.send(embed=discord.Embed(
                    title="🏆 Wins Leaderboard", description="No valid data found.", color=0xff0000
                ))
                return
            
            view = WinsLBView(rankings)
            embed = view.create_embed()
            await interaction.followup.send(embed=embed, view=view)
            
        except Exception as e:
            print(f"Error in wins_lb: {e}")
            try:
                await interaction.followup.send(embed=discord.Embed(
                    title="Error", description="Failed to fetch leaderboard.", color=0xff0000
                ))
            except:
                pass

async def setup(bot):
    await bot.add_cog(WinsLB(bot))
